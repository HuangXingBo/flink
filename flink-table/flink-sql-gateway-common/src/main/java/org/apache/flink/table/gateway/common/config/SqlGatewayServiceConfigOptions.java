...
public class SqlGatewayServiceConfigOptions {
    ...
    public static final ConfigOption<Duration> SQL_GATEWAY_EXECUTION_TIMEOUT =
            key("sql-gateway.execution.timeout")
                    .durationType()
                    .defaultValue(Duration.ofSeconds(120))
                    .withDescription(
                            "The call will be cancelled and throw an timeout exception when the execution time of call exceeds this value.");
    ...
    public static final ConfigOption<Integer> SQL_GATEWAY_WORKER_THREADS_MIN =
            key("sql-gateway.worker.threads.min")
                    .intType()
                    .defaultValue(5)
                    .withDescription("Minimum number of worker threads for gateway GRPC server.");
