...
public abstract class AbstractSqlGatewayRestHandler<
        ...
        extends AbstractHandler<RpcGateway, R, M> {

    private static final LocalLeader LOCAL_LEADER = new LocalLeader();
    ...
    protected AbstractSqlGatewayRestHandler(
        ...
        super(
                () -> CompletableFuture.completedFuture(LOCAL_LEADER),
                timeout,
                responseHeaders,
                messageHeaders);
    ...
    protected CompletableFuture<Void> respondToRequest(
        ...
        RestEndpointVersion version = RestEndpointVersion.valueOf(uri.substring(1, slashIndex));
    ...
    protected abstract CompletableFuture<P> handleRequest(
            RestEndpointVersion version, @Nonnull HandlerRequest<R> request)
            throws RestHandlerException;

    // --------------------------------------------------------------------------------------------

    private static class LocalLeader implements RpcGateway {

        private LocalLeader() {}

        @Override
        public String getAddress() {
            throw new UnsupportedOperationException("LocalLeader doesn't bind to any port.");
        }
